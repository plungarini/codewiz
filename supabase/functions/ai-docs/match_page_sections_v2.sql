...
-- [NEW] with dynamic table_name
create or replace function match_page_sections_v2(table_name text, embedding vector(1536), match_threshold float, min_content_length int)
...
begin
  return query
    ...
    from %I
		...
		-- We only care about sections that have a useful amount of content
    where length(%I.content) >= %s

		-- The dot product is negative because of a Postgres limitation, so we negate it
		and (%I.embedding <#> %L) * -1 > %s
		...
		--
		-- For the different syntaxes, see https://github.com/pgvector/pgvector
    order by %I.embedding <#> %L;
  ', table_name, min_content_length, embedding, match_threshold, embedding);
end;
$$;
